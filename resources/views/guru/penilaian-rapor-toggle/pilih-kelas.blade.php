...
<div>
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
        <div>
            ...
            <p class="page-subtitle">Semester: {{ $semester->nama }} ({{ $semester->tahunAjaran->nama ?? '-' }})</p>
        </div>
        <a href="{{ route('guru.penilaian-rapor-toggle.index') }}" class="btn-tartil-outline" style="text-decoration: none;">Kembali</a>
    </div>

    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px;">
        @forelse($kelasList as $kelas)
        <a href="{{ route('guru.penilaian-rapor-toggle.isi-nilai', [$semester->id, $kelas->id]) }}" 
           class="card-tartil" 
           ...
           onmouseout="this.style.transform='';this.style.boxShadow='';">
            ...
            <p style="font-size: 13px; color: var(--text-muted); margin: 0 0 12px 0;">
                {{ $kelas->siswas_count ?? 0 }} siswa aktif
            </p>
            ...
            <div style="margin-bottom: 8px;">
                ...
                <div style="width: 100%; height: 6px; background: var(--surface); border-radius: 3px; overflow: hidden;">
                    <div style="width: {{ $kelas->progress_persen }}%; height: 100%; background: var(--accent); border-radius: 3px; transition: width 0.3s;"></div>
                </div>
            </div>
