...
<div>
    ...
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
        ...
        <a href="{{ route('admin.manajemen.guru') }}" class="btn-tartil-outline">Kembali</a>
    </div>

    <div class="card-tartil" style="max-width: 700px; padding: 28px;">
        <form method="POST" action="{{ route('admin.manajemen.guru.update', $guru->id) }}">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px;">
                ...
                <div style="grid-column: span 2;">
                    ...
                    <input type="text" name="nama" class="form-input @error('nama') is-invalid @enderror" value="{{ old('nama', $guru->nama) }}" required style="width: 100%;">
                ...
                <div>
                    ...
                    <input type="email" name="email" class="form-input @error('email') is-invalid @enderror" value="{{ old('email', $guru->email) }}" required style="width: 100%;">
                ...
                <div>
                    ...
                    <input type="text" name="no_hp" class="form-input @error('no_hp') is-invalid @enderror" value="{{ old('no_hp', $guru->no_hp) }}" required style="width: 100%;">
            ...
            <div style="margin-top: 20px; padding: 12px; background: var(--bg-body); border-radius: 8px; font-size: 12px; color: var(--text-muted);">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                    <div>Dibuat: {{ $guru->created_at->format('d/m/Y H:i') }}</div>
                    <div>Terakhir diubah: {{ $guru->updated_at->format('d/m/Y H:i') }}</div>
                </div>
            </div>
            ...
            <div style="margin-top: 24px; display: flex; gap: 10px; padding-top: 16px; border-top: 1px solid var(--border);">
                ...
                <a href="{{ route('admin.manajemen.guru') }}" class="btn-tartil-outline" style="text-decoration: none;">Batal</a>
            </div>
        </form>
    </div>
</div>
