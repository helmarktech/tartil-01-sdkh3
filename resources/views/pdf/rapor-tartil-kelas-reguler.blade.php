...
<title>Rapor Kelas Reguler {{ $kelasReguler->nama }}</title>
<style>
...
.keputusan { font-size: 9px; text-align: center; padding: 8px 12px; margin: 8px 0; border: 1.5px solid #44403c; border-radius: 4px; line-height: 1.6; color: #1c1917; background: #fafaf9; display: block; }
.keputusan strong { color: #000; font-weight: 700; }
...
@foreach($rekapSemuaSiswa as $data)
@php
    $siswa = $data['siswa'];
    $rekap = $data['rekap'];
    $kelasTartil = $data['kelasTartil'];
@endphp
...
<table class="kop-table">
    <tr>
        <td style="width: 50px;">
            @if($kop->logo_dompdf)
            <img src="{{ $kop->logo_dompdf }}" class="kop-logo">
            @elseif($kop->logo_base64)
...
<table class="info-box">
    ...
    <tr>
        ...
        <td class="value">: {{ $kelasReguler->nama }} {{ $kelasReguler->jenjang ? '(' . $kelasReguler->jenjang . ')' : '' }}</td>
        <td class="label">Kelas Tartil</td>
        <td class="value">: {{ $kelasTartil->nama }} ({{ $kelasTartil->jenis }})</td>
    </tr>
    <tr>
        <td class="label">Guru Tartil</td>
        <td class="value">: {{ $kelasTartil->guru->nama ?? '-' }}</td>
...
<table class="ttd-table">
    <tr>
        <td>
            <div class="ttd-jabatan">Guru Tartil<br>{{ $kelasTartil->guru->nama ?? '........................' }}</div>
            <div class="ttd-nama">({{ $kelasTartil->guru->nama ?? '........................' }})</div>
        </td>
        <td style="position: relative;">
            @if($kop->stempel_dompdf)
            <img src="{{ $kop->stempel_dompdf }}" style="position: absolute; top: -10px; right: 10px; height: 70px; width: auto; opacity: 0.7; z-index: 1;">
            @elseif($kop->stempel_base64)
            ...
            @if($kop->ttd_dompdf)
            <img src="{{ $kop->ttd_dompdf }}" style="height: 35px; width: auto; margin: 2px 0;">
            @elseif($kop->ttd_base64)
